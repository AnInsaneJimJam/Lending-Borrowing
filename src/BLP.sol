...
contract BLP is ReentrancyGuard {
    uint256 private constant BORROWING_INTEREST_RATE = 8;
    uint256 private constant LENDING_INTEREST_RATE = 3;
    ...
    modifier validCollateralToken(address token) {
        require(
            token == addressOfWeth,
            BLP__InvalidCollateralTokenAddress(token)
        );
        _;
    }
    ...
    ////////////////  Public functions //////////////
    function provideUSDTForLiquidity(
        address token,
        uint256 amount
    ) public validDepositToken(token) moreThanZero(amount) {
        USDT.transferFrom(msg.sender, address(this), amount);
        userDeposits[msg.sender].push(Deposit(amount, block.timestamp));
        emit BLP_USDT_Deposited(amount);
    }

    function withdrawLiquidityInUsdt(
        uint256 amount
    ) public nonReentrant moreThanZero(amount) {
        uint256 totalValue = _getTotalValueOfDeposit(msg.sender); // Check it's msg.sender
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            uint256 interest = _calulateInterest(
                deposit.amount,
                timeElapsed,
    ...
    function withdrawCollateral(
        ...
        require(
            collateralDeposited[msg.sender] > amount,
            BLP__InsufficentCollateralToWithdraw()
        );
        collateralDeposited[msg.sender] -= amount;
    ...
    function borrowUsdt(
        address token,
        uint256 amount
    ) public moreThanZero(amount) validDepositToken(token) nonReentrant {
        userDebt[msg.sender].push(Debt(amount, block.timestamp));
        _revertIfColllaterizationRatioIsLow(msg.sender);
        require(
            USDT.balanceOf(address(this)) > amount,
            BLP__InsufficientLiquidity()
        );
    ...
    function getUsdValue(
        ...
        AggregatorV3Interface priceFeed = AggregatorV3Interface(
            token == addressOfUsdt ? usdtPriceFeed : wethPriceFeed
        );
    ...
    function getTotalValueOfDeposit()
        public
        view
        returns (uint256 totalAmount)
    {
        _getTotalValueOfDeposit(msg.sender);
    }

    /////////////// Internal functions /////////////

    function _calulateInterest(
        uint256 amount,
        uint256 timeElapsed,
    ...
    function _calculateCollaterizationRatio(
        address user
    ) internal view returns (uint256 ratio) {
        ratio =
            (getUsdValue(addressOfUsdt, collateralDeposited[user]) * 1e18) /
            getUsdValue(addressOfUsdt, _calculateTotalDebt(user));
    ...
    function _calculateTotalDebt(
        ...
        uint256 length = getDebts(msg.sender).length;
        for (uint256 i = 0; i < length; i++) {
            uint256 amount = getDebts(msg.sender)[i].amount;
            uint256 timestamp = getDebts(msg.sender)[i].timestamp;
    ...
    function _getTotalValueOfDeposit(
        address user
    ) internal view returns (uint256 totalAmount) {
    ...
    function getDebts(address user) public view returns (Debt[] memory) {
